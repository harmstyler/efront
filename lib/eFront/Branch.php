...
class Branch extends eFront
{
     ...
     */
    public function assignUserToBranch($token, $login, $branchId, $positionId, $jobDescription)
    {
        $xml_response = $this->buildResponse($this->apiBase . "?action=user_to_branch" . "&token=" . $token . "&login=" . $login . '&branch=' . $branchId . '&position=' . $positionId . '&job_description=' . $jobDescription);
        return $xml_response;
    }
